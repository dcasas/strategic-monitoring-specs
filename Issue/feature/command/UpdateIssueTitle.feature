Feature: UpdateIssueTitle Command
  ...
  Scenario: User should be able to create an issue
     When the (User) "<PERSONA>" executes the (UpdateIssueTitle) "command" with attributes
          | title  | 
          | ValTit |
     Then the (UpdateIssueTitle) "command" outcome is "SUCCESS"
    Examples:
        | PERSONA | LEVEL             |
