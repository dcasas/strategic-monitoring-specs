Feature: Some terse yet descriptive text of what is desired
   ...
   Scenario: Some determinable business situation
      Given there is a (DomainConcept2) "ref2" with attributes
        ...
        And there is a (User) "u" with (Role) "Not Enough Permissions"
       When the (User) "u" executes (ExampleQuery) "example" with attributes
            | otherAttrName1   | otherAttrName2 |
            | <Command1.attr1> | value2         |
       Then the (ExampleQuery) "example" outcome is "FAILED"
        And the (ExampleQuery) "example" outcome message is equals to "Not Authorized"
